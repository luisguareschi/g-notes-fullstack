...
export function SheetMenu({ config }: { config: SidebarLayoutProps }) {
  return (
    <Sheet>
      ...
      <SheetContent className="sm:w-72 px-3 h-full flex flex-col dark:border-zinc-800" side="left">
        <SheetHeader>
          <Button
          ...
          >
            <Link href={config.header.href} className="flex items-center gap-2">
              {config.header.icon}
              <SheetTitle className="font-bold text-lg">{config.header.title}</SheetTitle>
            </Link>
          </Button>
        </SheetHeader>
        <Menu
